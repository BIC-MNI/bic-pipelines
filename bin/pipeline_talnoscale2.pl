...
if($#ARGV < 3){ die "Usage: $me <path to mnc file> <path to xfm file> <path to transformed mnc file> <path to use for \"de-scaled\" xfm file>\n"; }
...
$scale=`xfm2param $infile_xfm|fgrep -- -scale`;
chomp($scale);
print "Scale: $scale\n" if $verbose;
...
print "Model: ", $modelfn, "\n" if $verbose;
do_cmd('mincresample', '-transformation', $outfile_xfm, '-like', $modelfn, $infile_mnc, $outfile_mnc,'-clobber');
