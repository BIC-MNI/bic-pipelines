...
# January, 2005: Modified for NIHPD's latest db structure.
...
print STDOUT "*** Pveing linear $subject_visit\n";
@args = ('pipeline_pve', "$subject_visit");
if($clobber){ push(@args, '-clobber'); }
if($verbose){ push(@args, '-verbose'); }
system(@args) == 0 or die;

print STDOUT "*** Pveing non linear $subject_visit\n";
@args = ('pipeline_pve', "$subject_visit", '-nl1');
if($clobber){ push(@args, '-clobber'); }
if($verbose){ push(@args, '-verbose'); }
system(@args) == 0 or die
    
print STDOUT "*** Pveing linear with clean $subject_visit\n";
@args = ('pipeline_pve', "$subject_visit", '-cls_algorithm', 'clean_cls');
if($clobber){ push(@args, '-clobber'); }
if($verbose){ push(@args, '-verbose'); }
system(@args) == 0 or die;

print STDOUT "*** Pveing non linear with clean$subject_visit\n";
@args = ('pipeline_pve', "$subject_visit", '-nl1', '-cls_algorithm', 'clean_cls');
if($clobber){ push(@args, '-clobber'); }
if($verbose){ push(@args, '-verbose'); }
system(@args) == 0 or die
