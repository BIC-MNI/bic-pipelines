...
if($#ARGV < 4) { die "Usage: $me <input tal mask> <input classified file> <input lobes file> <input xfm file> <path to output volumes text file> --age <n> --scanner <scanner> --scanner_id <n> --t1 <t1.mnc> --t2 <t2.mnc> --pd <pd.mnc>\n"; }
...
if (! -e $infile_lobes )      { die "$infile_lobes does not exist\n"; }
if (! -e $infile_xfm )        { die "$infile_xfm does not exist\n"; }
#if (! -e $infile_seg )        { die "$infile_seg does not exist\n"; }

#Build the temporary filenames.
my ($tmpfile_msk, $suffix, $basename);
($basename,$tmpfile_msk,$suffix) = fileparse($infile_classified,(".mnc.gz"));
$tmpfile_msk = $tmpfile_msk . $basename . "_tmpvolumemsk" . $suffix;
my $tmpfile_lobes;
($basename,$tmpfile_lobes,$suffix) = fileparse($infile_lobes,(".mnc.gz"));
$tmpfile_lobes = $tmpfile_lobes . $basename . "_tmplobesvolume" . $suffix;
...
print OUTFILE_VOLUMES "Gender $gender\n" if $gender;
