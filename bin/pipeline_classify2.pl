...
if ($#ARGV < 1){ die "Usage: $me <infile_t1> [infilet2] [infilepd] <outfile_clean_mnc> [--mask <mask file> --xfm <nl xfm> --mrf]\n"; }
...
if($mask_file && ! -e $mask_file)
{
  if(-e "$mask_file.gz")
  {
    $mask_file="$mask_file.gz";
  } else {
    die "Can't find mask file $mask_file\n";
  }
}
